...
const Blog = () => {
    ...
    return data ? (
        <div className={'relative'}>
            <img src={assets.gradientBackground} alt="bachground" className={'absolute -top-50 -z-1 opacity-50'}/>
            <Navbar/>
            <div className={'text-center mt-20 text-gray-600'}>
                <p className={'text-primary py-4 font-medium'}>Published On {Moment(data.createdAt).format('MMMM Do YYYY')}</p>
                <h1 className={'text-2xl sm:text-5xl font-semibold max-w-2xl mx-auto text-gray-800'}>{data.title}</h1>
                <h2 className={'my-5 mx-w-lg truncate mx-auto'} dangerouslySetInnerHTML={{__html:data.subTitle}}></h2>
                <p className= 'inline-block py-1 px-4 rounded-full mb-6 border text-sm border-primary/35 bg-primary/5 font-medium text-primary'>STCC</p>
            </div>
                ...
                <div className={'max-w3xl mx-auto'}>
                    ...
                    <form onSubmit={addComment} className={'flex flex-col gap-4 items-start max-w-lg'}>
                        ...
                        <textarea onChange={(e) => setContent(e.target.value)} value={content}
                            placeholder={'Comment'} required className={'w-full p-2 border border-gray-300 rounded outline-none h-48'}/>
